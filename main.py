...
import os
import boto3
from database import init_db, get_db, User, Note, Transaction, NoteLike, NoteDownload, NoteShare, CouponCode
...
def get_earnings(token: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "message": f"₹0.10 per download. {total_downloads} downloads = ₹{round(total_earnings, 2)}"
    }
...
@app.post("/api/user/apply-coupon")
def apply_coupon(request: dict, token: str, db: Session = Depends(get_db)):
    user_id = int(verify_token(token))
    coupon_code = request.get('coupon_code', '').strip()
    coupon = db.query(CouponCode).filter(CouponCode.code == coupon_code, CouponCode.is_active == 1).first()
    if not coupon:
        raise HTTPException(status_code=404, detail="Invalid coupon code")
    user = db.query(User).filter(User.id == user_id).first()
    user.is_premium = 1
    user.premium_expires_at = datetime.utcnow() + timedelta(days=coupon.days)
    db.commit()
    return {"message": f"Premium activated for {coupon.days} days!", "is_premium": True, "expires_at": user.premium_expires_at.isoformat()}

@app.get("/api/user/check-premium")
def check_premium(token: str, db: Session = Depends(get_db)):
    user_id = int(verify_token(token))
    user = db.query(User).filter(User.id == user_id).first()
    if user.is_premium and user.premium_expires_at and datetime.utcnow() > user.premium_expires_at:
        user.is_premium = 0
        db.commit()
        return {"is_premium": False}
    return {"is_premium": bool(user.is_premium), "expires_at": user.premium_expires_at.isoformat() if user.premium_expires_at else None}
...
async def upload_note(file: UploadFile = File(...), title: str = Form(...), subject: str = Form(...), description: str = Form(...), token: str = Form(...), db: Session = Depends(get_db)):
    user_id = int(verify_token(token))
    user = db.query(User).filter(User.id == user_id).first()
    user_notes_count = db.query(Note).filter(Note.user_id == user_id).count()
    max_uploads = 10 if user.is_premium else 3
    if user_notes_count >= max_uploads:
        raise HTTPException(status_code=403, detail=f"Upload limit reached. {'Premium' if user.is_premium else 'Free'} users can upload max {max_uploads} notes.")
    ...
    db.commit()
    db.refresh(note)
    return {"message": "Note uploaded", "note": {"id": note.id, "title": note.title, "url": s3_url}, "uploads_remaining": max_uploads - user_notes_count - 1}
...
def download_note(note_id: int, token: str, db: Session = Depends(get_db)):
    ...
    if not user.is_premium:
        raise HTTPException(status_code=403, detail="Premium membership required to download notes")
    ...
    if not existing_download:
        db.add(NoteDownload(note_id=note_id, user_id=user_id))
        note.downloads += 1
        note.earnings += 0.1
        db.add(Transaction(note_id=note.id, buyer_id=user_id, amount=0.1))
        db.commit()
...
def share_note(note_id: int, token: str, db: Session = Depends(get_db)):
    ...
    if not existing_share:
        db.add(NoteShare(note_id=note_id, user_id=user_id))
        note.shares += 1
        db.commit()
